...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint_interval", type=int, default=2, help="Interval between saving model checkpoints"
    )
    ...
    train_stat_df = pd.DataFrame(columns=['Epoch','Training Loss','Training Accuracy','Validation Loss','Validation Accuracy'])
    ...
    for epoch in range(opt.num_epochs):
        ...
        # Evaluate the model on the test set
        test_epoch_loss, test_epoch_accuracy = test_model(epoch)

        training_loss_list.append(np.mean(epoch_metrics["loss"]))
        training_accuracy_list.append(np.mean(epoch_metrics["acc"]))

        validation_accuracy_list.append(test_epoch_accuracy)
        validation_loss_list.append(test_epoch_loss)
    ...
    print('Epoch :', len(epoch_list))
    print('Training Loss Length :', len(training_loss_list))
    print('Training Accuracy Length :', len(training_accuracy_list))
    print('Validation Loss Length :', len(validation_loss_list))
    print('Validation Accuracy Length :', len(validation_accuracy_list))
            
    train_stat_df['Epoch'] = epoch_list
    train_stat_df['Training Loss'] = training_loss_list
    train_stat_df['Training Accuracy'] = training_accuracy_list
    train_stat_df['Validation Loss'] = validation_loss_list
    train_stat_df['Validation Accuracy'] = validation_accuracy_list
    train_stat_df.to_csv ('train_statistics.csv', index = None, header=True,sep='\t')
